...
# Rebuilt: full-precision shadow model, 2-decimal display, ▲ marker, stable load/save/events
...
def load_any_excel(path: str, app_instance=None):
    ...
    try:
        if ext in [".xlsx", ".xlsm"]:
            ...
            # limited image detection (best-effort)
...
def normalize_numeric(value: str, fmt: str = "integer"):
    """
    Parse numeric input but DO NOT round away full precision.
    Return int for exact integers (if integer format), else float for decimals.
    """
...
def round_half_up(value, decimal_places=2):
    """
    Excel-style rounding (0-4 down, 5-9 up), no bankers' rounding.
    """
...
class DynamicExcelApp:
    def __init__(self, root: Window):
        ...
        # display_row_index starts at 1 for first data row in the view
        self.shadow_values = {}
    ...
    def _on_entry_tab(self, event):
        # Move focus to next entry; return "break" to stop default tab behavior when needed
        try:
            widget = event.widget
            if widget in self.input_entries:
                idx = self.input_entries.index(widget)
                if idx + 1 < len(self.input_entries):
    ...
    def new_file(self):
        # Create a new in-memory workbook and reset UI
        wb = Workbook()
        ws = wb.active
        ws.title = "Sheet1"
        self.workbook = wb
    ...
    def _create_bottom_frame(self):
        ...
        # Bind some handlers (some are re-bound in _bind_events)
    ...
    def open_file(self):
        ...
        if ext not in [".xlsx", ".xlsm"]:
            ...
            messagebox.showinfo("Format Notice",
    ...
    def _load_active_sheet(self):
        ...
        rows = []
        # clear shadows for current sheet
        ...
        for excel_row in range(start_row, sheet.max_row+1):
            ...
            if all(v == "" or v is None for v in rowvals):
                continue
            # populate shadow for each cell: raw=cell.value, parsed=cell.value, rounded_flag=False
    ...
    def _build_input_fields(self, headers):
        ...
        for idx, header in enumerate(headers):
            ...
            ttk.Radiobutton(control_frame, text="Warn (Allow Duplicates)", variable=rule['duplicate_var'], value="warn", command=lambda r=rule: self._update_validation_state(r)).pack(anchor=tk.W, padx=10)
            ttk.Radiobutton(control_frame, text="No Duplicates (Strict)", variable=rule['duplicate_var'], value="strict", command=lambda r=rule: self._update_validation_state(r)).pack(anchor=tk.W, padx=10)
    ...
    def add_row_from_inputs(self):
        ...
        for idx, entry in enumerate(self.input_entries):
            ...
            field_name = self.input_order[idx] if idx < len(self.input_order) else f"Column {idx+1}"
            rule = self.current_rules.get(field_name, {})
            display_text = format_value_for_display(parsed, rule=rule)
            rounded_flag = detect_precision_mismatch(raw, parsed, decimal_places=2) if parsed is not None and rule.get("format") == "decimal" else False
    ...
    def update_row_from_inputs(self):
        ...
        is_valid, strict_messages, warning_messages, normalized = self.validate_inputs()
        if not is_valid:
            self._update_status(f"Strict validation failed on {len(strict_messages)} fields. See red fields.", "error")
            return
        if warning_messages:
            warning_text = "\n".join(warning_messages)
            prompt = (
        ...
        self.unsaved_changes = True
        self._update_status(f"Updated row {excel_row - 1} successfully.", "success")
        self.tree.selection_set(self.editing_item)
        self.tree.see(self.editing_item)
        if hasattr(self, 'auto_save_var') and self.auto_save_var.get():
            if self.save_file():
                self._update_status("Auto-saved after editing row ✅", "success")
        self.reset_to_add_mode()
    ...
    def _flush_shadow_to_workbook(self):
        if not self.workbook:
            return
        for (sheet_name, display_row, col_idx), payload in list(self.shadow_values.items()):
            try:
                if sheet_name not in self.workbook.sheetnames:
                    continue
                sheet = self.workbook[sheet_name]
                excel_row = display_row + 1  # header at row 1
                sheet.cell(row=excel_row, column=col_idx).value = payload.get("value")
            except Exception:
                pass
    ...
    def save_file(self):
        ...
        try:
            self._flush_shadow_to_workbook()
    ...
    def on_theme_change(self, event=None):
        try:
            ...
            self._update_status(f"Theme changed to {self.theme_combo.get()}", "success")
        except Exception as e:
            messagebox.showerror("Theme error", f"Cannot set theme {self.theme_combo.get()}:\n{e}")
    ...
    def _show_temp_warning(self, message, duration=5000):
        ...
        try:
            ...
            pass
        msg = ttk.Label(popup, text=message, wraplength=380, justify="center", foreground="#8a6d3b", background="#fff4e5", font=("Segoe UI",10,"bold"))
    ...
    def _show_help(self):
        ...
        if not os.path.exists(help_path):
            messagebox.showinfo("Help File Missing", "No 'help.txt' file found in the app directory or bundled resources.")
            return
        try:
        ...
        text_widget = tk.Text(text_frame, wrap="word", font=("Segoe UI",10), relief="flat", bg="#ffffff", fg="#333333")
